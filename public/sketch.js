...
function assess(player, computer) {
	// Tell the server the final choices
	socket.emit('player', player);
	socket.emit('computer', computer);
...
function drawScreen() {	
	...
	switch (stage)  {
		...
		case "finale":
			...
			// Broadcast result to server
			socket.emit('result', win_lose_draw);
			socket.emit('pscore', player_score);
			socket.emit('cscore', computer_score);
